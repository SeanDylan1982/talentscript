...
import { Mail, Phone, MapPin, Globe, Linkedin, Github, BookOpen, GraduationCap, Award, Briefcase, FileText, Book } from 'lucide-react';
...
export function AcademicTemplate({ data }: TemplateProps) {
  ...
  const renderSection = (section: typeof sections[0]) => {
    switch (section.type) {
      case 'personalInfo':
        return (
          <div key={section.id} className="bg-white p-6 border-b border-gray-200">
      ...
      case 'publications':
        // Publications would be part of the projects array with type 'publication'
        const publications = projects.filter(proj => proj.type === 'publication');
            ...
            <div className="space-y-4">
              {publications.map((pub) => (
                <div key={pub.id} className="break-inside-avoid">
                  ...
                  <p className="text-xs text-gray-700 mt-0.5" style={{ fontFamily: customization.fontFamily }}>
                    {pub.description}
                  </p>
                  {pub.technologies && pub.technologies.length > 0 && (
                    <div className="mt-1 text-xs text-gray-600">
                      <span className="font-medium">Keywords:</span>{' '}
                      <span style={{ fontFamily: customization.fontFamily }}>
      ...
      case 'projects':
        // Filter out publications as they're handled separately
        const nonPublicationProjects = projects.filter(proj => proj.type !== 'publication');
        if (nonPublicationProjects.length === 0) return null;
